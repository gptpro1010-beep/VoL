...
/* ============================================================
   Compute nearest-neighbor distance for one snapshot
   ============================================================ */
void compute_nnd(const std::vector<float>& xx,
...
int main()
{
    ...
    fs::create_directories("../../part_3d_properties_data/part_3a_nnd_nosingle");
    ...
    for (float spd : speed_vals)
    {
        ...
        #pragma omp parallel
        {
            ...
            std::vector<float> xx(N), yy(N), nnd;
        ...
        std::snprintf(outname,sizeof(outname),
            "../../part_3d_properties_data/part_3a_nnd_nosingle/"
            "nnd_nosingle_avg_rho_%.2f_speed_%.2f_noise_%.2f.txt",
            rho,spd,noise);

        std::ofstream out(outname);

        out << "# avg NND(k): mean ± std across simulations\n";
