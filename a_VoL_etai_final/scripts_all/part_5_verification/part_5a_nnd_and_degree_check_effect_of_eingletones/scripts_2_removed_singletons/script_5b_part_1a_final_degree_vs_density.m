...
finalK = nan(numel(speeds), numel(rhos));
...
for iSpeed = 1:numel(speeds)
    ...
    for iRho = 1:numel(rhos)
        ...
        finalK(iSpeed,iRho) = mean(meanK(end-9:end));
...
for iSpeed = 1:numel(speeds)
    ...
    hList(iSpeed) = plot(ax, rhos, finalK(iSpeed,:), ...
        'LineWidth',2, ...
        'Color',cc(iSpeed,:));
...
%% SAVE
filename = fullfile(outDir,'part_1a_final_degree_vs_density_multi_speed_nosingle.png');
