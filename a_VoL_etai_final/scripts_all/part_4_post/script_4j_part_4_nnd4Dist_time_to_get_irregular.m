...
title(ax, sprintf('NND4 time to loss regularity (q = %.1f)', q), ...
      'Interpreter','latex','FontSize',16);

box(ax,'on');

%% Save
outDir = fullfile('figs','figs_4j_part_4_nnd4Dist_time_to_get_irregular');
if ~isfolder(outDir), mkdir(outDir); end

fname = fullfile(outDir, sprintf('nnd4_time_to_irregularity_q_%.2f.png', q));
print(gcf, fname, '-dpng', '-r600');

disp('NND4 time-to-irregularity phase diagram generated.');
