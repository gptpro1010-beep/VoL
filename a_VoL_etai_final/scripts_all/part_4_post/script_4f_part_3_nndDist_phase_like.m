...
%% Output
outDir = fullfile('figs','figs_4f_part_3_nnd_phase_like');
...
for ik = 1:numel(kvals)
    ...
    for iRho = 1:numel(rhos)
        ...
        for iSpeed = 1:numel(speeds)
            ...
            % columns: snap_idx  r  mean_F  std_F  mean_d
            snap_all = dat(:,1);
            r_all    = dat(:,2);
            F_all    = dat(:,3);
    ...
    title(ax, sprintf('NND phase (q = %.1f) at $T/T_{cross} = %d$', ...
        q, 5*snap_idx), ...
    ...
    fname = fullfile(outDir, ...
        sprintf('nnd_phase_rho_speed_Tcross_%06d.png', 5*snap_idx));
    print(gcf, fname, '-dpng', '-r600');
    close(gcf);

    fprintf('Done NND phase t = %d\n', 5*snap_idx);
end

disp('NND regular/clustered phase diagrams generated.');