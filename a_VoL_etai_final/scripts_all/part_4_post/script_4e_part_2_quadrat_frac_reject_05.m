...
outDir = fullfile('figs','figs_4e_part_2_quadrat_frac_reject_05');
if ~isfolder(outDir)
    mkdir(outDir);
end
...
for ik = 1:numel(kvals)
    ...
    FRAC05 = nan(numel(speeds), numel(rhos));
    ...
    title(ax, ...
        sprintf('$Fraction\\ rejecting\\ CSR\\ (5\\%%)$ at $T/T_{cross}=%d$',5*k_sel), ...
    ...
    fname = fullfile(outDir, ...
        sprintf('phase_quadrat_reject05_rho_speed_Tcross_%05d.png',5*k_sel));
...
disp('Quadrat reject05 phase diagrams generated.');