...
% Data location
dataDir = '../part_3d_properties_data/part_3a_nnd';

outDir = fullfile('figs','figs_4a_nnd');
if ~isfolder(outDir)
    mkdir(outDir);
end
...
for iRho = 1:numel(rhos)
    ...
    for iSpeed = 1:numel(speeds)
        ...
        % Plot
        hList(iSpeed) = plot(ax, Tnorm, meanNND, ...
            'Color', col, ...
            'LineStyle', ls, ...
            'LineWidth', 1.8);
    ...
    ylabel('$NND$', 'Interpreter','latex','FontSize',16);
    ...
    % Remove empty legend entries if some files missing
    ...
    filename = fullfile(outDir, ...
        sprintf('nnd_vs_Tcross_rho_%.2f.png', rho));
