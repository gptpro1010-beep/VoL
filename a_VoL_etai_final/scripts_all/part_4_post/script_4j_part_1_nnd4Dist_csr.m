...
for b = 1:B
    ...
    H4_sim(b,:) = arrayfun(@(tt) mean(d4 <= tt), t_s);
...
%% CSR envelope
H4_low  = quantile(H4_sim, 0.025, 1);
H4_high = quantile(H4_sim, 0.975, 1);

%% Output directory
outDir = fullfile('data', 'data_4j_nnd4Dist_csr');
if ~isfolder(outDir)
    mkdir(outDir);
end

outFile = fullfile(outDir, ...
    sprintf('csr_envelope_nnd4_dist_periodic_N_%d_B_%d.txt', N, B));
