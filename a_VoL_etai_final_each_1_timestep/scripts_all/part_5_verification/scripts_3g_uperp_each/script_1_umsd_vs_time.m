...
dataDir = '../../part_3d_properties_data/part_3g_uperp_each';
...
for iSpeed = 1:numel(speeds)
    ...
    for iRho = 1:numel(rhos)
        ...
        for iSim = 1:numel(sim_nums)
            ...
            %% plot
            % loglog(ax, t, u_msd, 'Color', cc(iSpeed,:), 'LineWidth', 2,'LineStyle','-');
            plot(ax, t, u_msd, 'Color', cc(iSpeed,:), 'LineWidth', 3, 'LineStyle','-');
            ...
            fill([t; flipud(t)], [upper; flipud(lower)], cc(iSpeed,:), 'FaceAlpha', 0.15, 'EdgeColor', 'none');
