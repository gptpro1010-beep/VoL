...
outDir = fullfile('figs','figs_4a_lcc_size_time');
if ~isfolder(outDir)
    mkdir(outDir);
end
...
for iSpeed = 1:numel(speeds)
    ...
    for iRho = 1:numel(rhos)
        ...
        plot(ax, t, m_mean, 'LineWidth',2, ...
            'Color', colors(iRho,:), ...
            'DisplayName', sprintf('$\\rho = %g$', rho));

        % errorbar(ax, t, m_mean, m_std, 'LineStyle','none', 'Color', colors(iRho,:), 'CapSize', 0);
    end
    ...
    ylabel(ax, '$m_C$', 'Interpreter','latex','FontSize',16);

    title(ax, sprintf('$m_C(t)$ at $s=%.2f$', s), 'Interpreter','latex','FontSize',16);
    ...
    fname = sprintf('lcc_size_vs_time_speed_%.2f.png', s);
